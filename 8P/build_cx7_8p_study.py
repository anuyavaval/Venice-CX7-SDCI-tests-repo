#!/usr/bin/env python3
"""
Build CX7-8C-study-VeniceB0-G2-BIOS.xlsx  (SDCI ENABLED)
  Tab 1 "System"      — HW/NIC/BIOS/FW + 8P settings + iperf3 commands
  Tab 2 "8-Core Study"— aggregate -P sweep table (SDCI ENABLED) + per-core breakdown
Reads the newest results_* dir (or RESULT_DIR env override).
Output: 8P/results/CX7-8C-study-VeniceB0-G2-BIOS.xlsx
"""
import os, csv, glob
import openpyxl
...
OUT = os.path.join(OUTDIR, "CX7-8C-study-VeniceB0-G2-BIOS.xlsx")
...
title(ws,1,1,2,"CX7 8-Core iPerf Study — System Configuration",bg=AMD_RED,sz=14,ht=30)
...
for k,v in [("Device","NVIDIA/Mellanox ConnectX-7 (MT2910)"),("Part Number","MCX75310AAS-NEA_Ax"),
    ...
    kv(ws,r,k,v); r+=1
r+=1
title(ws,r,1,2,"8P (8-CORE) NIC SETTINGS",bg=HDR_BLUE,sz=11); r+=1
for k,v in [
...
for k,v in [("Tool","iperf3 — 8 concurrent processes, one per core; -P streams per process"),
...
r=cmd(r,"LoadGen  8 servers", f"for p in {ports}: taskset -c <core> iperf3 -s -B 192.168.10.3 -p <p>")
r=cmd(r,"SUT  8 clients (concurrent)",
...
r=cmd(r,"  force 8Q combined", f"ethtool -L eth2 combined {NUM_CORES}")
...
# ── Tab 2: 8-Core Study ──
ws2=wb.create_sheet("8-Core Study"); ws2.sheet_view.showGridLines=False
...
title(ws2,2,1,ncol,
      f"eth2 (CX7 400G) | 8Q | iperf cores {CORES[0]}-{CORES[-1]}, IRQs interleaved to siblings "
...
if plist:
    notes=[
        ...
        f"8 iperf3 procs on cores {CORES[0]}-{CORES[-1]}; 8Q; 64 IRQs interleaved across siblings "
        f"{SIBLINGS[0]}-{SIBLINGS[-1]} via i%{NUM_CORES}.",
        "Per-core columns show load balance across the 8 cores.",
        "SDCI ENABLED. Compare against 1P (single-core ~107 Gbps SDCI-on) to see 8-core scaling.",
    ]
    for n in notes:
