#!/usr/bin/env python3
"""
CX7 8-Core (8P) iPerf core-scaling orchestrator — SDCI ENABLED.

Extends the 1P study to 8 cores. Runs FROM the Windows workstation, drives both
lab boxes over SSH (paramiko).

Per the parent-directory scaling convention:
  - 8 iperf3 client processes, one pinned per core (cores 5..12).
  - eth2 forced to combined = 8 queues (ethtool -L eth2 combined 8).
  - The NIC's 64 IRQs are INTERLEAVED across the 8 SMT sibling cores (261..268)
    using modulo:  sibling = SIBLINGS[irq_index % NUM_CORES].
    So IRQ0->261, IRQ1->262, ... IRQ7->268, IRQ8->261, ...  (round-robin).
  - Each of the 8 procs runs the SAME -P value; we sweep -P over the list.
  - 10 iterations per -P, 60 s each. Throughput = SUM of the 8 procs (aggregate),
...
# 8 iperf3 servers on the loadgen, ports BASE_PORT .. BASE_PORT+7 (one per core).
...
def main():
    ...
    print(f"8P: cores {CORES} | siblings {SIBLINGS} | ports {PORTS}\n")
    ...
    print("\n== LoadGen: tuning + starting 8 iperf3 servers ==")
    ...
    # ── SUT: tuning + combined=8 + interleaved IRQ steering ──
    print("\n== SUT: tuning eth2 (8Q, interleaved IRQs) ==")
    ...
    # spot-check first 9 completion IRQs
    chk, _ = run(sut, f"for n in $(seq 0 8); do "
    ...
    def one_stream(port, core, P):
        """Run one iperf3 client (own SSH channel) and return (gbps, retrans)."""
        cc = connect(SUT)
        try:
            cmd = (f"taskset -c {core} iperf3 -c {SRV_DATA_IP} -p {port} "
                   f"-P {P} -t {DURATION} -J")
            out, _ = run(cc, cmd, timeout=DURATION + 30)
            j = json.loads(out)
    ...
    for P in P_LIST:
        ...
        for it in range(1, ITERS + 1):
            # launch all 8 procs concurrently
            with ThreadPoolExecutor(max_workers=NUM_CORES) as ex:
                res = list(ex.map(lambda t: one_stream(*t, P),
                                  list(zip(PORTS, CORES))))
