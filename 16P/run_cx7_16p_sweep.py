#!/usr/bin/env python3
"""
CX7 16-Core (16P) iPerf core-scaling orchestrator — SDCI ENABLED.

Extends the 1P/8P study to 16 cores. Runs FROM the Windows workstation, drives
both lab boxes over SSH (paramiko).

Per the parent-directory scaling convention (core-count is config-driven):
  - NUM_CORES iperf3 client processes, one pinned per core (cores 1..16, skip 0).
  - eth2 forced to combined = NUM_CORES queues (ethtool -L eth2 combined 16).
  - The NIC's 64 IRQs are INTERLEAVED across the SMT sibling cores (257..272)
    using modulo:  sibling = SIBLINGS[irq_index % NUM_CORES].
    With 64 IRQs / 16 cores that is 4 IRQs per sibling (round-robin).
...
def main():
    ...
    print(f"16P: cores {CORES} | siblings {SIBLINGS} | ports {PORTS}\n")
    ...
    # Opening a fresh SSH connection per stream per iteration (16 at once) trips
    # sshd's MaxStartups rate limit ("Error reading SSH protocol banner"). Instead
    # open ONE connection per core up front (staggered), then reuse them for every
    # iteration — exec_command multiplexes channels over the existing transport, so
    # no new TCP/SSH handshakes happen during the sweep.
