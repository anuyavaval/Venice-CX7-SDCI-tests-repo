#!/usr/bin/env python3
"""
Build CX7-16C-study-VeniceB0-G2-BIOS.xlsx  (SDCI ENABLED)
  Tab 1 "System"      — HW/NIC/BIOS/FW + 16P settings + iperf3 commands
  Tab 2 "16-Core Study"— aggregate -P sweep table (SDCI ENABLED) + per-core breakdown
Reads the newest results_* dir (or RESULT_DIR env override).
Output: 16P/results/CX7-16C-study-VeniceB0-G2-BIOS.xlsx
"""
import os, csv, glob
import openpyxl
...
OUT = os.path.join(OUTDIR, "CX7-16C-study-VeniceB0-G2-BIOS.xlsx")
...
title(ws,1,1,2,"CX7 16-Core iPerf Study — System Configuration",bg=AMD_RED,sz=14,ht=30)
...
for k,v in [("Device","NVIDIA/Mellanox ConnectX-7 (MT2910)"),("Part Number","MCX75310AAS-NEA_Ax"),
    ...
    kv(ws,r,k,v); r+=1
r+=1
title(ws,r,1,2,"16P (16-CORE) NIC SETTINGS",bg=HDR_BLUE,sz=11); r+=1
for k,v in [
...
# ── Tab 2: 16-Core Study ──
ws2=wb.create_sheet("16-Core Study"); ws2.sheet_view.showGridLines=False
...
title(ws2,1,1,ncol,"CX7 16-Core iPerf Core-Scaling — SDCI ENABLED",bg=AMD_RED,sz=14,ht=30)
title(ws2,2,1,ncol,
...
title(ws2,4,1,ncol,"16-CORE STUDY — Aggregate throughput + per-core breakdown (SDCI ENABLED)",bg=HDR_BLUE,sz=11)
hr=5
...
if plist:
    notes=[
        ...
        f"{SIBLINGS[0]}-{SIBLINGS[-1]} via i%{NUM_CORES} (4 IRQs per sibling).",
        f"Per-core columns show load balance across the {NUM_CORES} cores.",
        "SDCI ENABLED. Compare against 1P (~107 Gbps) and 8P (~330 Gbps) to see scaling.",
    ]
    for n in notes:
