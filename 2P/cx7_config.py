...
# ── Test knobs ──
IPERF_CORE = int(os.environ.get("CX7_IPERF_CORE", "5"))   # first core (1P) / base core
...
BASE_CORE      = int(os.environ.get("CX7_BASE_CORE", "1"))   # cores 1..2, skip core 0
NUM_CORES      = int(os.environ.get("CX7_NUM_CORES", "2"))   # 2 cores / procs / queues
SIBLING_OFFSET = int(os.environ.get("CX7_SIBLING_OFFSET", "256"))  # sibling = core+256 (CCD0)

CORES    = list(range(BASE_CORE, BASE_CORE + NUM_CORES))          # [1, 2]
SIBLINGS = [c + SIBLING_OFFSET for c in CORES]                    # [257, 258]
